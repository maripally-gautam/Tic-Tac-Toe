import React from "react";
import { motion } from "motion/react";

interface Spark {
  id: number;
  x: number;
  y: number;
  angle: number;
  distance: number;
  color: string;
  size: number;
}

export default function ParticleExplosion() {
  // Generate beautiful sparks bursting outwards
  const colors = [
    "#FF5722", // Neon Orange
    "#FFEB3B", // Neon Yellow
    "#4CAF50", // Neon Green
    "#E91E63", // Hot Pink
    "#00BCD4", // Neon Cyan
    "#00E676"  // Coral Lime
  ];

  const sparks: Spark[] = Array.from({ length: 45 }, (_, idx) => {
    const angle = Math.random() * Math.PI * 2;
    const distance = 80 + Math.random() * 160;
    return {
      id: idx,
      x: 0,
      y: 0,
      angle,
      distance,
      color: colors[idx % colors.length],
      size: Math.random() * 7 + 4
    };
  });

  return (
    <div className="absolute inset-0 pointer-events-none z-40 flex items-center justify-center overflow-visible">
      {sparks.map((spark) => {
        const destX = Math.cos(spark.angle) * spark.distance;
        const destY = Math.sin(spark.angle) * spark.distance + 80; // gravity fall offset

        return (
          <motion.div
            key={spark.id}
            initial={{ x: 0, y: 0, scale: 0.8, opacity: 1 }}
            animate={{
              x: destX,
              y: destY,
              scale: 0,
              opacity: 0,
              rotate: Math.random() * 360
            }}
            transition={{
              duration: 1.2 + Math.random() * 0.4,
              ease: [0.1, 0.8, 0.25, 1] // swift explosion profile
            }}
            style={{
              position: "absolute",
              width: spark.size,
              height: spark.size,
              backgroundColor: spark.color,
              borderRadius: spark.id % 2 === 0 ? "50%" : "2px",
              boxShadow: `0 0 10px ${spark.color}, 0 0 20px ${spark.color}`
            }}
          />
        );
      })}
    </div>
  );
}
